...
// src
import {CrioArray, CrioObject} from '../src/Crio';
...
test(
  'if getRelativeValue returns a number between the 0 and the length, which is equal to the value if it falls in that ' +
    'range, and the difference of the length and the number when it is less than 0',
  (t) => {
    const length = 10;
    const inValue = 5;
